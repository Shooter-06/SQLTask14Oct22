...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        db.execSQL("create table Myproduct (id integer primary key autoincrement, name vachar" +
                ", brand varchar,prc number, rev vachar )");
    }

    @Override
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        db.execSQL("Drop table if exists Myproduct");
        onCreate(db);
    }
    ...
    public boolean addProducts(String name, String brand, String price, String reviews){
        ...
        db.insert("Myproduct", null, contentValues);
        db.close();
        return true;
    }
